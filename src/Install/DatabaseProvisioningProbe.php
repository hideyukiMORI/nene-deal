...
/**
 * Defence-in-depth for {@see \Nene2\Install\ReInstallationGuard}: reports whether
 * the configured database already holds a provisioned Deal, so a lost `.installed`
 * marker (e.g. an ephemeral `var/` wiped on redeploy) cannot let the installer
 * overwrite `.env` or recreate the admin account.
 *
 * Deal-specific nuance: the migrations themselves seed a development operator
 * (`operator@nene-deal.test`), so right after `phinx migrate` the `users` table is
 * never empty. A target counts as provisioned only when a user OTHER than that
 * dev seed exists — i.e. a real admin was created. This keeps a half-finished
 * install (migrated, but admin creation failed) retryable instead of locked out.
 *
 * Reads the connection from a parsed `.env`; supports both adapters Deal ships
 * with (sqlite / mysql). Returns `false` — never throws — when there is no `.env`,
 * the DB is unreachable, or the schema is absent, so a genuinely fresh target is
 * not mistaken for a provisioned one.
 */
final class DatabaseProvisioningProbe implements ProvisioningProbe
{
    ...
    public static function fromEnvFile(string $envPath, string $projectRoot): self
    {
        ...
        // (#78). An unreadable `.env` degrades to an empty env — the marker layer
        // of the guard still holds — and never produces output.
        $env = [];
    ...
    public function isProvisioned(): bool
    {
        ...
        try {
            ...
            if ($statement === false || !$statement->execute([AdminProvisioner::DEV_SEED_EMAIL])) {
                return false;
            }
    ...
    private function connect(): ?PDO
    {
        ...
        $dbName = $this->env['DB_NAME'] ?? 'var/nene_deal.sqlite';
