 ...
 * not, so neither `.htaccess` tricks nor `CGIPassAuth` can help; proven on
 * the invoice and vault deployments, fleet form of invoice #597 / vault
 * #118). The SPA mirrors the token into `X-Authorization`; that value is
 * adopted only when `Authorization` is absent, so environments that deliver
 * the standard header are unaffected.
 */
