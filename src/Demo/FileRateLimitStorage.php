 ...
 * (`Nene2\Demo` consumer, #69 — ported from the proven invoice/clear concrete).
 *
...
final readonly class FileRateLimitStorage implements RateLimitStorageInterface
{
    ...
    public function hit(string $key, int $windowSeconds): array
    {
        ...
        if ($handle === false) {
            error_log(sprintf('NeNe Deal: could not persist rate-limit state at %s; this hit is not counted against the window.', $file));
