/* NeNe Deal — セットアップウィザード クライアント挙動 (#65)
 *
 * CSP（script-src 'self'）下で動くよう外部ファイル化している（インライン不可）。
 * 役割: パスワード表示切替 / DB アダプタ切替 / ホストプリセット & コントロール
 * パネル図 / 送信時のローディング（実際の疎通確認 + サブステップ進捗アニメーション）。
 * 挙動は nene-vault の installer.js と同型（deal に不要な部品は削除）。
 */
(function () {
  'use strict';
  ...
  Array.prototype.forEach.call(document.querySelectorAll('.host-chip'), function (chip) {
    chip.addEventListener('click', function () {
      ...
      if (cpDb) { cpDb.textContent = d.db || 'yourname_deal'; }
  ...
  /* ---------- 送信時のローディング（疎通確認 + 進捗） ---------- */
  // フォーム別サブステップ。実際の作業（接続・.env・スキーマ・管理者作成）は
  // サーバー側で 1 リクエストにまとまっているため、各行はアニメーション上の
  // 段階表現。成否は POST 応答の data-error から正しく反映する。
  var SUBSTEPS = {
    dbForm: [
    ...
    appForm: [
      ...
      { t: '組織と管理者を作成しています', d: '初期アカウントを差し替え中' }
    ]
  };
  var STEP_MS = 720;
