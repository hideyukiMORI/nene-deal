...
// 🔴 5187, not Vite's default 5173 — `vite.config.ts` pins `port: 5187` with `strictPort`
// (AGENTS.md "Local dev ports").
const LOCAL_URL = process.env.NENE_DEAL_OWNER_REVIEW_LOCAL_URL ?? 'http://localhost:5187'
...
async function assertSeated(page: Page, where: string): Promise<void> {
  ...
  expect(shell, `${where}: not seated — the app rendered the login screen`).toBeGreaterThan(0)
}
 ...
 *
 * 🔴 Checked on `/settings`, and the screen was chosen by **measuring**, not by analogy.
 *
 * The first version guarded on `/` (the board), copying vault's reasoning that the landing
 * screen is the product's own markup and the first list screen is where the kit appears.
 * That is false here. Measured against a seeded local build on 2026-08-26:
 *
 *     board     x-slot elements =  0   (822 classes on the page)
 *     users     x-slot elements =  4
 *     stages    x-slot elements =  6
 *     settings  x-slot elements =  5
 *
 * deal's board reaches the kit only **conditionally** — `EmptyState` when there are no
 * columns, `Badge` for a won or deleted deal — and the demo seed happens to produce neither.
 * So a fully styled build fails the guard there, and the run dies before it photographs
 * anything. `/settings` renders its `Badge` and its `FormField`+`Select` unconditionally.
 *
 * 🔑 A guard that asks "is the kit present?" has to be pointed at a screen where the kit is
 * present **regardless of the data**. That is a property of this product, and no other
 * ship's spec can tell you which screen it is.
 */
async function assertStyled(page: Page, side: string): Promise<void> {
  const slots = await page.locator('[class*="x-slot"]').count()
  expect(slots, `${side}: no kit slot utility on /settings — unstyled build?`).toBeGreaterThan(0)
}
...
async function shootSide(
  ...
  page.on('request', (req) => {
    ...
    if (seat === null && auth !== undefined && auth.startsWith('Bearer ')) seat = auth.slice(7)
  })

  await page.goto('/demo/standard', { waitUntil: 'networkidle' })
  await page.locator('header.topnav, header.m-topbar').first().waitFor({ timeout: 30_000 })

  if (seat === null) {
    throw new Error(`${side}: seated, but no Bearer went past — cannot re-seat later loads`)
  }
...
function kitVersion(rootDir: string): string {
  try {
    const p = resolve(rootDir, '../../../frontend/node_modules/@hideyukimori/nene2-ui/package.json')
  ...
  const rows = VIEWPORTS.flatMap((vp) =>
    SCREENS.map((sc) => {
      const at = (side: 'prod' | 'local') =>
        shots.find((s) => s.side === side && s.screen === sc.name && s.viewport === vp.name)
...
test('OWNER-REVIEW: production vs local, every screen, two viewports', async ({
  ...
  const prodBase = (testInfo.project.use.baseURL ?? 'https://deal.ayane.co.jp').replace(/\/$/, '')
  ...
  const meta: Record<string, string> = {
    ...
    'local worktree': sh('git status --porcelain') === '' ? 'clean' : '🔴 DIRTY (uncommitted)',
