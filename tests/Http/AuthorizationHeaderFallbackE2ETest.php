 ...
 *
 * `GET /api/v1/auth/me` is bearer-protected and, being the first middleware in the
 * auth stage's downstream chain, isolates the credential-restoration behaviour: the
 * assertions below only depend on whether `BearerTokenMiddleware` issued a
 * `WWW-Authenticate` challenge, never on what MeHandler does afterwards (this test
 * environment has no migrated schema, so a resolved token can still surface a 500
 * from the unmigrated `users` table — NENE2's ErrorHandlerMiddleware sits ahead of
 * the auth stage and never sets `WWW-Authenticate`, so that in no way confuses the
 * assertions).
 *
...
{
    private const PROTECTED_PATH = '/api/v1/auth/me';
     ...
     * challenge; its absence proves authentication succeeded (any further error here
     * is downstream of the auth stage — MeHandler resolving a user against an
     * unmigrated schema — which is out of scope for the transport-level mirror
     * proof).
     */
