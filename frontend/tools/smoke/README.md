...
Matrix: `{login, board, deal-detail} × {calm, enterprise} × {light, dark}`.
...
`.shell-ent` (enterprise shell markup needs a design _state_ switch, not just the
attribute), mobile sheet, `.board.is-dragging` (drag-transient) and the Account-menu
