...
// Playwright smoke harness — fleet T2 pilot (Issue #157).
//
...
// boot/login/nav check; the nene2-e2e harness will supersede it (T2).
//
...
const E2E_PORT = Number(process.env.NENE_DEAL_E2E_PORT) || 5189
const BASE_URL = process.env.NENE_DEAL_E2E_BASE_URL ?? `http://localhost:${E2E_PORT}`
...
export default defineConfig({
  ...
  // (§7 裁定4); the Playwright config and its node deps stay here in frontend/.
  // Because the specs sit above frontend/node_modules, the `e2e` npm script sets
  ...
  // NENE_DEAL_E2E_BASE_URL (e.g. staging) is assumed already running.
  webServer: process.env.NENE_DEAL_E2E_BASE_URL
    ? undefined
    : {
